...
contract Healthcare is ERC721 {
    ...
    mapping(address => bool) public hasNFT; // Track if the user has an NFT
    ...
    function deposit() external payable {
        ...
        // Mint an NFT only if the user has not received one yet
        if (!hasNFT[msg.sender]) {
            mint(msg.sender);
            hasNFT[msg.sender] = true; // Mark the user as having received an NFT
        }
    }
